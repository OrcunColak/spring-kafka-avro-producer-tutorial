...
@SpringBootTest
@Testcontainers
class KafkaAvroProducerWithConsumerTest {
    ...
    static void overrideProperties(DynamicPropertyRegistry registry) {
        ...
        registry.add("spring.kafka.properties.schema.registry.url", KafkaAvroProducerWithConsumerTest::getSchemaRegistryUrl);
    }
    ...
    void testWithConsumer() {
        ...
        kafkaAvroProducer.publish(event);
        ...
        try (KafkaConsumer<MyKeyAvro, MyEventAvro> consumer = new KafkaConsumer<>(consumerProperties)) {
            consumer.subscribe(Collections.singletonList(KafkaAvroProducer.TOPIC_NAME));
            Awaitility.await()
